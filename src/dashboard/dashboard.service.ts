...
export class DashboardService {
    ...
    async totalNetworkers(
        jwtPayload: JWTPayload,
    ) {
        try {
            const pipeline: any = await this.totalNetsResult(
    ...
    async totalVistorsLastSixMonths(
        jwtPayload: JWTPayload,
    ) {
        try {
            ...
            const totalvisitorsLastSixMonths = await this.usersModel.aggregate(pipeline);
