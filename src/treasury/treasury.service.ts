...
export class TreasuryService {
    ...
    async create(treasuryDTO: TreasuryDTO): Promise<ServicesResponse> {
        ...
        try {
            ...
            if (findPayment == null) {
                treasuryDTO = { ...treasuryDTO, chapterId: ObjectId(treasuryDTO.chapterId), userId: ObjectId(treasuryDTO.userId) };
                ...
                if (paymentCreated != null) {
                    ...
                    this.emailProperties.email = findUser.email;
                    this.emailProperties.name = findUser.name + ' ' + findUser.lastName;
                    this.emailProperties.amount = treasuryDTO.payment.toString() + ' pesos de ' + treasuryDTO.monthYear;
                    this.emailProperties.template = process.env.RECEIPT_TEMPLATE;
                    this.emailProperties.subject = process.env.SUBJECT_RECEIPT;
    ...
    async userPaymentList(id: string): Promise<ServicesResponse> {
        ...
        try {
            const paymentList = await this.treasuryModel.find({
                userId: ObjectId(id),
                status: 'Active'
            }, {
                _id: 0,
                payment: 1, monthYear: 1,
                paymentDate: 1
            }).sort({ createdAt: 1 });
